<?php
    ...
    if (empty($exe_nome) || empty($ser_serie)) {
        header("location: ../criarTreino.php?membro=Membros Inferiores&situacaoMen=faltando&situacao=conectado&id=". $_GET['id']."");
        exit();
    } else {
            ...
            if ($result_series_banco->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt = $conexao->prepare("SELECT 
                        ...
                        WHERE u.usu_id = $usuario_id AND t.membro_nome = 'Membros Inferiores';");
                    $stmt->execute();
                    ...
                    if ($result_treino->num_rows > 0) {
                        ...
                        if ($stmt->execute()) {
                            // Sucesso na inserção
                            header("location: ../criarTreino.php?membro=Membros Inferiores&situacaoMen=sucesso&situacao=conectado&id=". $_GET['id']."");
                            exit();
                        } else {
                        ...
                        echo "Nenhum treino encontrado para o membro 'Membros Inferiores'.";
                    }
                } else {
