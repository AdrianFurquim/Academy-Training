<?php
    ...
    if (empty($exe_nome) || empty($ser_serie)) {
        header("location: ../criarTreino.php?membro=Membros Inferiores&situacao=faltando");
        exit();
    } else {
        ...
        if ($result_exercicios_banco_esp->num_rows > 0) {
            ...
            if ($result_series_banco->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt = $conexao->prepare("SELECT * FROM treinos WHERE membro_nome = 'Membros Inferiores';");
                    $stmt->execute();
                    ...
                    if ($result_treino->num_rows > 0) {
                        ...
                        if ($stmt->execute()) {
                            // Sucesso na inserção
                            header("location: ../criarTreino.php?membro=Membros Inferiores&situacao=sucesso");
                            exit();
                        } else {
