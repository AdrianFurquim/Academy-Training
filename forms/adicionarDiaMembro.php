<?php
    ...
    if (empty($dia) || empty($membro)) {
        ...
        // Consulta no banco de dados para verificar se o membro já existe nos dados do usuário.
        ...
        WHERE 
            u.usu_id = ? AND t.membro_nome = ?");
        $stmt->bind_param("is", $usuario_id, $membro);
        $stmt->execute();
        $result_treinos_membro_banco_esp = $stmt->get_result();
        $stmt->close();

        // Caso não exista nenhuma resposta, significa que o usuário não possui um membro com este exercício.
        if ($result_treinos_membro_banco_esp->num_rows <= 0) {

            // Consulta no banco de dados para verificar se o dia já existe nos dados do usuário.
            $stmt = $conexao->prepare("SELECT 
                DISTINCT t.dia_nome AS Dia,
                t.membro_nome AS Membro, 
                t.tre_id AS TreinoId, 
            ...
            JOIN 
                treinos t ON te.treino_id = t.tre_id
            WHERE 
                u.usu_id = ? AND t.dia_nome = ?");
            $stmt->bind_param("is", $usuario_id, $dia);
            $stmt->execute();
            $result_treinos_dia_banco_esp = $stmt->get_result();
            $stmt->close();

            // Caso não exista nenhuma resposta, significa que o usuário não possui um dia com este exercício.
            if ($result_treinos_dia_banco_esp->num_rows <= 0) {
