<?php
    ...
    if (empty($exe_nome) || empty($ser_serie)) {
        header("location: ../criarTreino.php?membro=Abdominal&situacaoMen=faltando&situacao=conectado&id=". $_GET['id']."");
        exit();
    } else {
        ...
        if ($result_exercicios_banco_esp->num_rows > 0) {
            ...
            if ($result_series_banco->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    WHERE u.usu_id = $usuario_id AND t.membro_nome = 'Abdominal';");
                    $stmt->execute();
                    ...
                    if ($result_treino->num_rows > 0) {
                        ...
                        if ($stmt->execute()) {
                            // Sucesso na inserção
                            header("location: ../criarTreino.php?membro=Abdominal&situacaoMen=sucesso&situacao=conectado&id=". $_GET['id']."");
                            exit();
                        } else {
